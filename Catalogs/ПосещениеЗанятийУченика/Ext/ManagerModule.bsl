...
//
Функция ДанныеПроизводственногоКалендаря(РасписаниеЗанятий, НомерГода) Экспорт
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("РасписаниеЗанятий",	РасписаниеЗанятий);
	Запрос.УстановитьПараметр("ТекущийГод",	НомерГода);
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ДанныеРасписанияЗанятий.Дата,
		|	ДанныеРасписанияЗанятий.ВидДня,
		|	ДанныеРасписанияЗанятий.ДатаПереноса
		|ИЗ
		|	РегистрСведений.ДанныеРасписанияЗанятий КАК ДанныеРасписанияЗанятий
		|ГДЕ
		|	ДанныеРасписанияЗанятий.Год = &ТекущийГод
		|	И ДанныеРасписанияЗанятий.РасписаниеЗанятий = &РасписаниеЗанятий
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата";
...
Функция ЦветаОформленияВидовДнейПроизводственногоКалендаря() Экспорт
	...
	ЦветаОформления.Вставить(Перечисления.ВидыДнейРасписания.Занятия,			ЦветаСтиля.ВидДняПроизводственногоКалендаряРабочийЦвет);
	ЦветаОформления.Вставить(Перечисления.ВидыДнейРасписания.Праздник,			ЦветаСтиля.ВидДняПроизводственногоКалендаряПраздникЦвет);
	ЦветаОформления.Вставить(Перечисления.ВидыДнейРасписания.НетЗанятий,		ЦветаСтиля.ВидДняПроизводственногоКалендаряСубботаЦвет);
...
Функция СписокВидовДня() Экспорт
	...
	Для Каждого МетаданныеВидаДней Из Метаданные.Перечисления.ВидыДнейРасписания.ЗначенияПеречисления Цикл
		СписокВидовДня.Добавить(Перечисления.ВидыДнейРасписания[МетаданныеВидаДней.Имя], МетаданныеВидаДней.Синоним);
	КонецЦикла;
